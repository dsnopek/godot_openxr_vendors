...
func _on_timer_timeout() -> void:
	...
	pivot_tween.tween_method(self._tween_pivot_transform_basis, _pivot_transform.basis.get_rotation_quaternion(), new_basis.get_rotation_quaternion(), 1.0) \
		.set_trans(Tween.TRANS_EXPO) \
		.set_ease(Tween.EASE_OUT)
	pivot_tween.parallel().tween_method(self._tween_pivot_transform_origin, _pivot_transform.origin, Vector3(_current_position.x, 0.0, _current_position.z), 1.0) \
		.set_trans(Tween.TRANS_EXPO) \
		.set_ease(Tween.EASE_OUT)
